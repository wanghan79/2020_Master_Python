...
def data_sampling(datatype, datarange, num, strlen=6): # 固定参数；可变参数arg*；默认参数；关键字参数**kwargs
...
def data_screening(dataset, condition):
    '''
...
def apply():
    result = data_sampling(int, (1,100), 10) | data_sampling(str, "aasbttttttaaaaoqmalzeropf", 10, )
    print('目标数据:', result)
    print('---------筛选出0,50之间的数字和含有at的字符串--------------')
    real_result = data_screening(result, (0, 50, "at"))
    print('筛选后数据：', real_result)
